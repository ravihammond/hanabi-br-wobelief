...
void HandDistribution::updateOccurance(
  ...
  std::tie(numOccurances_, sumOccurance_) = computeNumOccurance(
      allHands_,
      cardCount,
      [rank = numRank_](const hle::HanabiCardValue& card) { return cardToIndex(card, rank); },
      allowZero);
...
void HandDistribution::filterExact(std::vector<hle::HanabiCardValue> hand, size_t extraHand) {
  std::vector<std::vector<hle::HanabiCardValue>> newHands;
  std::vector<int> newOccurances;
  int64_t newSum = 0;
...
void HandDistribution::filterCounterfactual(
  ...
  auto forwardModel = [&](int begin, int end) {
    ...
    for (int i = begin; i < end; ++i) {
      ...
      GameSimulator simGame(state, {{meIndex, mySimCards}});
...
void updateBelief(
  ...
  if (lastMove == nullptr) {
    ...
    assert(handInBelief(myHand, myIndex, handDist));
    ...
    if (lastMove->player != 0 && !skipCounterfactual) {
      ...
      handDist.filterCounterfactual(
          myIndex,
          partner.index,  // partnerIdx,
          game.GetMoveUid(lastMove->move),
          partner,
          *prevState,
          numThread);
      assert(handInBelief(myHand, myIndex, handDist));
      std::cout << "filtering counterfactual, size: "
                << after << "->" << handDist.size() << std::endl;
    }
  }
