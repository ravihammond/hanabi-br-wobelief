...
def parse_args():
    ...
    parser.add_argument("--load_model", type=str, default="../sad/models/sad_wov0_embedding/model4.pthw")
    ...
    parser.add_argument("--burn_in_frames", type=int, default=300)
...
if __name__ == "__main__":
    ...
    logger_path = os.path.join(args.save_dir, "train_wov0.log")
    ...
    index = 0
    for i in range(100000 // args.batchsize):
        batch_sum = 0
        print("Avg, " + str(i+1) + "/" + str(100000 // args.batchsize))
        batch, weight = replay_buffer.sample(args.batchsize, args.train_device)
        score = batch.reward.min(dim = 0).values * (-1)
        score = score[score > 0]
        game_scores = np.append(game_scores, score.cpu().data.numpy())
        index += 1
        avg += torch.sum(score)
    ...
    dev = 0
    # deviations = np.zeros(len(game_scores))
    for i in range(len(game_scores)):
        print("Dev, " + str(i+1) + "/" + str(len(game_scores)))
        # deviations[i] = np.abs(avg - game_scores[i])
        dev += (avg - game_scores[i])**2
    # dev = np.median(deviations)
    dev /= len(game_scores)
    dev = dev**0.5
    
    print(game_scores)

    print("Average score: " + str(avg) + " ± " + str(dev))
    assert 1 == 2
    ...
    #         force_save_name = "model_wov0_epoch_%d" % epoch
    #     else:
    #         force_save_name = None
    #     model_saved = saver.save(
    #         # None, agent.online_net.state_dict(), score, force_save_name=force_save_name
    #         None, agent.online_net.state_dict(), stat.stats['loss'].summation / stat.stats['loss'].counter, force_save_name=force_save_name
    #     )
    #     # print(
